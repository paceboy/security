...
### 2.9 根据RFID获取安检记录详情
**GET** `/security/record/rfid/{rfidCode}`

**功能说明**: 根据RFID标识获取最近一条安检记录的完整详情，包含关联的人脸、RFID、包裹照片、X光照片、身份证、违禁品等所有信息

**路径参数**:
| 字段名 | 类型 | 必填 | 说明 |
|-------|------|-----|------|
| rfidCode | String | Y | RFID标识 |

**响应参数**: 与 2.2 接口响应参数完全一致
...
```json
{
  ...
  "data": {
    "id": 123,
    "recordNo": "SC20250127120000001",
    "rfidCode": "04E050014FE5778B",
    "checkTime": "2025-01-27 12:00:00",
    "checkStatus": 0,
    "checkStatusText": "待登记",
    "conclusionType": "高危",
    "faceRecord": {...},
...
# 根据RFID获取安检记录详情
curl -X GET \
  "http://192.168.88.94:8080/security/record/rfid/04E050014FE5778B" \
  -H "Authorization: Bearer <your_token>"
```

**业务逻辑说明**:
1. 根据传入的 `rfidCode` 在 `t_security_check_record` 表中查询
2. 按 `create_time` 降序排列，取最近一条记录
3. 复用 2.2 接口的逻辑，获取该记录的完整详情（包含所有关联数据）
4. 如果未找到匹配的记录，返回 `data: null`
